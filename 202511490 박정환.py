...
# 엑셀 파일 이름을 지정합니다.
FILE_NAME = 'C:/Users/Jhp06/Univ2nd/vkdlTjs/weedahan/student_data.xlsx'
# 파일 경로에 대해서는 주도와 보조 의 경로 지정 방식이 달라 추후 통합 예정입니다.
...
def read_excel_file(file_name):
    ...
    try:
        ...
        if '학번' in data_table.columns and '학점' in data_table.columns:
            print("\n--- '학번', '이름', '성별', '성적', '학점' 컬럼 ---")
            print(data_table[['학번', '이름', '성별', '성적', '학점']])
        
    except Exception as e:
        print(f"엑셀 파일을 읽는 중 오류가 발생했습니다. 라이브러리가 제대로 설치되어 있는지 확인해주세요: {e}")
